...
must_haves:
  ...
  artifacts:
    - path: "app/src/services/concept-feed.service.ts"
      provides: "Card-face-only batch prompt, relaxed validators"
      contains: "Do NOT include bodyMarkdown"
    - path: "app/src/services/post-essay.service.ts"
      provides: "On-enter essay generation dispatcher"
...
4. In `getPostById()` function (~line 881):
   - After the `getConnectionPostFromStore(id)` fallback (line 886), add additional lookups for video and news caches:
     ```typescript
     // Check video cache
     try {
...
5. IMPORTANT: Leave `STARTER_POSTS` array unchanged — these have hardcoded full bodyMarkdown and should NOT go through on-enter generation. The on-enter trigger in PostDetailScreen (Plan 03) will check `post.bodyMarkdown !== ''` so starters are safe.
  ...
  <acceptance_criteria>
    ...
    - concept-feed.service.ts getPostById contains `echolearn_video_cache` and `echolearn_news_posts` lookups
    - STARTER_POSTS still contain full bodyMarkdown strings (not empty)
  </acceptance_criteria>
  <done>Batch generation prompt requests only card-face fields; validators accept posts with empty bodyMarkdown; getPostById searches all caches</done>
</task>
 ...
 * Handles AI posts (echolearn_daily_posts), video (echolearn_video_cache), news (echolearn_news_posts).
 */
...
3. In `news.service.ts`, in `generateNewsPosts()` (line 67):
  ...
  <done>On-enter essay generation service created with type-specific generators; video and news batch generation defers LLM calls; essay cache patching handles all storage keys</done>
</task>
...
- getPostById searches video, news, and shorts caches
