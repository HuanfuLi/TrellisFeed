...
- [x] **Phase 1: Rebrand + research shell hardening** - Branded shell with condition config + interaction logging plumbing (done 2026-07-11)
- [x] **Phase 2: Content pool + feed/post UI on frozen data** - Real curated frozen pool browsable, with post-scoped Ask (completed 2026-07-18)
- [x] **Phase 3: Graph-memory + recommendation engine** - Two conditions produce different-but-comparable feeds with interpretable reasons (completed 2026-07-18)
- [x] **Phase 4: Study infrastructure + pilot** - Onboarding, condition assignment, data export, oral-test support; internal pilot; IRB-ready (completed 2026-07-19)
...
- [x] 01-01-PLAN.md — Rebrand display/native/locale surfaces; preserve bundle IDs (SHELL-01)
- [x] 01-02-PLAN.md — Storage namespace rename to `questiontrace` + research-store schema; remove legacy migration (SHELL-02)
- [x] 01-03-PLAN.md — Backend: Cloudflare Worker + D1 ingest/resolve + validation (idempotent, field-allowlisted) (LOG-01)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 01-04-PLAN.md — Research types + immutable study-context (condition plumbing) + App hydration gate (SHELL-03)
- [x] 01-05-PLAN.md — Backend: password-protected admin page + CSV/ZIP export + deploy checkpoint (LOG-01)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 01-06-PLAN.md — Durable upload queue + retry + upload-health metadata (LOG-01)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 01-07-PLAN.md — Interaction-log service: whitelisted events + revisioned Q/A records (LOG-01)
- [x] 01-09-PLAN.md — Participant surface reduction + PIN-gated researcher diagnostics + recovery export (SHELL-04, LOG-01)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 01-08-PLAN.md — Instrumentation wiring at call sites + RQ-01 coverage (LOG-01, RQ-01)
- [x] 01-10-PLAN.md — Dead-code sweep: pruned residue removed, load-bearing infra preserved (SHELL-04)

**Gap closure waves**

- [x] 01-11-PLAN.md — Consent/auth/CORS and deployed collector hardening
- [x] 01-12-PLAN.md — Crash-safe outbox, quarantine, and reconciliation
- [x] 01-13-PLAN.md — Rebrand residue, English default, zero-failure gates, and physical-device smoke
...
- [x] 03-01-PLAN.md — Pipeline typed §10.4 global-graph + ranking-features compiler into the freeze contract (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 03-02-PLAN.md — App graph types, IDB v6 stores, bundle import, type-indexed global-graph repository (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 03-03-PLAN.md — Personal graph-memory: idempotent §10.6 contribution ledger, UserConceptState, replay/snapshot (wave 3)
- [x] 03-04-PLAN.md — Pure rankers: §11.7 control, §11.3/§11.4 experimental, §11.5 strategies, §11.6 diversity + §12.3 tests 1–4 (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 03-05-PLAN.md — Async §17.2 extraction with frozen-ID allowlist + RQ-02 wire/backend extension (wave 4)
- [x] 03-06-PLAN.md — Single recommendation seam: condition branch, persisted batches, §17.4 reasons + §12.3 tests 5–6 (wave 4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 03-07-PLAN.md — Home/FeedCard on recommendation batches, per-item impressions, reason surface (wave 5)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 03-08-PLAN.md — D-12 guarded removal sweep: transitional shell, pinning tests, IDB v7 store cleanup (wave 6)

**Gap closure waves**

- [x] 03-09-PLAN.md — Unify the nine-artifact freeze hash contract and create a new immutable graph pool (wave 7)
- [x] 03-10-PLAN.md — Make pool selection reviewable and cut generated/native assets over to all nine artifacts (wave 8)
- [x] 03-11-PLAN.md — Gate App boot on global-graph loading and add fresh-freeze-to-first-batch regression coverage (wave 9)
...
**Success Criteria** (what must be TRUE):
  ...
  1. A participant completes onboarding (welcome, language, §14.3 consent, study-provided LLM); the single topic is server-bound with no topic choice, and the researcher-assigned condition is persisted and drives the ranker + logging (D-01/D-02/D-03).
  2. Researchers can export analyzable logs, user questions, AI answers, and recommendations, with all §14.2 categories excluded.
  3. Pre/post oral explanations are captured and transcribed through the external researcher protocol (D-06); the export's `participants.csv` join key links them, and §13.4 scoring plus §13.5 normalization are supported by exported data and the external scoring sheet.
...
- [x] 04-01-PLAN.md — Recommendation wire contract, D1 migration 0004, backend validation (STUDY-03)
- [x] 04-04-PLAN.md — Versioned §14.3 research consent + five-item onboarding disclosure in 4 locales (STUDY-01)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 04-02-PLAN.md — Worker recommendation ingest + four-file export ZIP with participants manifest (STUDY-03, STUDY-04, RQ-03)
- [x] 04-03-PLAN.md — Client recommendation research projection through the durable outbox (STUDY-03)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-05-PLAN.md — STUDY-02 assignment traceability + D-15 doc wording alignment (STUDY-02)
- [x] 04-06-PLAN.md — Pilot protocol/checklist + blocking operator deploy gate (STUDY-05, STUDY-04, RQ-03)
...
| 1. Rebrand + research shell hardening | 13/13 | Complete | 2026-07-11 |
| 2. Content pool + feed/post UI on frozen data | 9/9 | Complete    | 2026-07-18 |
| 3. Graph-memory + recommendation engine | 11/11 | Complete    | 2026-07-19 |
| 4. Study infrastructure + pilot | 6/6 | Complete   | 2026-07-19 |
