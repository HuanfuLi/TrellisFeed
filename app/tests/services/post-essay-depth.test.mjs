...
//
// Covers SC-3 (depth: 'deep' produces 350-600w; standard bands preserved per
// surviving generator), SC-6 (generateEssayMeta body slice cap raised
// 2000→4000), AND behavioral merge tests for patchPostEssayInCache.
//
...
test('SC-3: surviving generators present in post-essay.service.ts', () => {
  for (const name of ['generateStandardEssay', 'generateTextArtEssay']) {
    assert.ok(SRC.includes(name), `generator ${name} must exist`);
  }
  for (const name of ['generateVideoEssay', 'generateNewsEssay']) {
    assert.ok(!SRC.includes(name), `removed generator ${name} must stay absent`);
  }
});
...
test("SC-3: depth knob branched in each generator (options?.depth ?? 'standard' pattern)", () => {
  ...
  assert.ok(matches.length >= 2, `expected ≥2 depth branches, got ${matches.length}`);
});

test('SC-3: deep band 350-600 referenced in surviving generators', () => {
  const matches = [...SRC.matchAll(/350-600/g)];
  assert.ok(matches.length >= 2, `expected ≥2 occurrences of 350-600, got ${matches.length}`);
});
...
test('counterweight: each generator threads options?.signal into chatStream', () => {
  ...
  // depth knob lands. Count 'signal: options?.signal' occurrences — at least 3
  // (2 generators + generateEssayMeta).
  const matches = [...SRC.matchAll(/signal: options\?\.signal/g)];
  assert.ok(matches.length >= 3, `expected ≥3 signal-threaded calls, got ${matches.length}`);
});
