...
The frozen participant bundle stores and displays complete article text or complete video transcripts. Public availability alone does not grant permission to reproduce a complete work. For each candidate, the rights reviewer must record one of these bases before operator approval:
...
If none applies, reject the candidate for full-text storage. Do not infer permission from a URL, an accessible page, a visible YouTube transcript, or the nonprofit/research character of the project.
...
- The official [YouTube Data API captions.download documentation](https://developers.google.com/youtube/v3/docs/captions/download) requires authorization sufficient to edit the video. It is not a public-caption download API for arbitrary third-party videos. Consequently, Phase 2 must not use unofficial endpoints or bypass controls. Use only creator-supplied/openly licensed transcripts, written permission, or operator-provided transcript files with a documented rights basis.
...
- approved participant-content delivery method (bundled full text/transcript versus source embed/link);
